...
void dgemm (int n, double* A, double* B, double* C){
 for (int i = 0; i < n; i += 8)
    for (int j = 0; j < n; ++j)
    {
        __m512d cij = _mm512_load_pd(C + i+ j*n); /* cij = C[i][j] */

        for( int k = 0; k < n; k++ )

        cij = _mm512_add_pd(cij, /* cij += A[i][k]*B[k][j] */
        _mm512_mul_pd(_mm512_load_pd(A+i+k*n),
        _mm512_broadcastsd_pd(_mm_set_sd(B[k+j*n]))));

        _mm512_store_pd(C+i+j*n, cij); /* C[i][j] = c0 */
    }
}
